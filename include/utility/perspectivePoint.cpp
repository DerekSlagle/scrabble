...
{
    x = ( xs - persPt::Xv )*persPt::Z0/( zs - persPt::Zv ) + persPt::X0;// x on screen - X0
}


void persPt::updateScreenPos()
{
    if( zs < persPt::Z0/2.0f + persPt::Zv ) return;// too close

    vec3f camPos( persPt::Xv, persPt::Yv, persPt::Zv ), ps( xs, ys, zs );
  //  vec3f camPos( 0.0f, 0.0f, 0.0f ), ps( xs, ys, zs );
    vec3f tmpPos = ( ps - camPos ).rotate_axis( vec3f(0.0f,1.0f,0.0f), angPolar );
    ps = tmpPos + camPos;

    float zFactor = Z0/( ps.z - persPt::Zv );
    x = ( ps.x - persPt::Xv )*zFactor + persPt::X0;
    y =  Y_0( ps.z ) - ( ps.y - persPt::Yv )*zFactor;
 //   y =  Y_0( ps.z ) - ( ps.y - persPt::Yv )*Z0/sqrtf( (ps.x-Xv)*(ps.x-Xv) + (ps.y-Yv)*(ps.y-Yv) + (ps.z-Zv)*(ps.z-Zv) );
}
...
void persPt::updateScreenPos()
{
    ...
    float zFactor = Z0/( zs - persPt::Zv );
    x = ( xs - persPt::Xv )*zFactor + persPt::X0;
    y =  Y_0( zs ) - ( ys - persPt::Yv )*zFactor;

    x += ( zs - persPt::Zv )*sinf( angPolar );
}   */


float persPt::zFact( float y )// y on screen
{
...
{
    if( pos.z < persPt::Z0/2.0f + persPt::Zv ) return;// too close
...
void persMatrix::setVtxPositions()
{
...
void persQuad::setPosition( float Xs, float Ys, float Zs )
{
    ...
    pt[0].init( pos + v*H );// up lt      v*H
    pt[1].init( pos + v*H + u*W );// up rt  v*H + u*W
    pt[2].init( pos + u*W );// bot rt     u*W
    pt[3].init( pos );// bot lt         0,0

    setVtxPositions();
    setDistSq();
}

void persQuad::setVtxPositions()
{
    for( unsigned int i = 0; i < 4; ++i ) vtx[i].position = pt[i].screenPos();
}
...
// persQuadAni
bool persQuadAni::fromFile( std::istream& is, spriteSheet& rSS )
{
    is >> pos.x >> pos.y >> pos.z;
    pos.z += persPt::Z0;
    is >> W >> H;
    ...
    frIdx = delayCnt = 0;
    char xf1, xf2; is >> xf1 >> xf2;
    setTxtRect( rSS.getFrRect( frIdx, setNum ), xf1, xf2 );
    return is.good();
}

void persQuadAni::update()// load next frame
{
    persPack::update();
    if( ++delayCnt > frDelay )
    {
        setTxtRect( pSS->getFrRect( frIdx, setNum, true ) );
        delayCnt = 0;
    }
}
...
void persBoxMover::updateMove( float dt )
{
    pos += v*dt;
    setPosition( pos.x, pos.y, pos.z );
}
